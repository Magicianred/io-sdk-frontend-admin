...
class Dialog {
    constructor(type, message, onHandle, defaultValue) {
        ...
        debugLogger_1.debugLogger.log('api', `  ${this._preview()} was shown`);
    }
    type() {
        return this._type;
    }
    ...
    async accept(promptText) {
        ...
        this._handled = true;
        debugLogger_1.debugLogger.log('api', `  ${this._preview()} was accepted`);
        await this._onHandle(true, promptText);
    }
    async dismiss() {
        utils_1.assert(!this._handled, 'Cannot dismiss dialog which is already handled!');
        this._handled = true;
        debugLogger_1.debugLogger.log('api', `  ${this._preview()} was dismissed`);
