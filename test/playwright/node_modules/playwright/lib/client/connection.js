...
class Connection {
    ...
    async sendMessageToServer(type, guid, method, params) {
        const stackObject = {};
    ...
    _createRemoteObject(parentGuid, type, guid, initializer) {
        ...
        switch (type) {
            ...
            case 'BrowserContext': {
                const browserName = initializer.browserName;
                if (browserName === 'chromium')
                    result = new chromiumBrowserContext_1.ChromiumBrowserContext(parent, type, guid, initializer);
                else
                    result = new browserContext_1.BrowserContext(parent, type, guid, initializer, browserName);
                break;
            }
            case 'BrowserType':
...
const tChannel = (name) => {
    return (arg, path) => {
        if (arg._object instanceof channelOwner_1.ChannelOwner && (name === '*' || arg._object._type === name))
            return { guid: arg._object._guid };
        throw new validator_1.ValidationError(`${path}: expected ${name}`);
    };
};
const scheme = validator_1.createScheme(tChannel);
function validateParams(type, method, params) {
