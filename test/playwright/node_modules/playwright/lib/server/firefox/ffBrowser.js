...
class FFBrowser extends browser_1.Browser {
    constructor(connection, options) {
        ...
        this._eventListeners = [
            ...
            helper_1.helper.addEventListener(this._connection, 'Browser.screencastFinished', this._onScreencastFinished.bind(this)),
        ];
    }
    ...
    _onAttachedToTarget(payload) {
        ...
        ffPage.pageOrError().then(async (pageOrError) => {
            ...
            if (pageOrError instanceof Error)
                page._setIsError();
            context.emit(browserContext_1.BrowserContext.Events.Page, page);
            if (!opener)
                return;
            const openerPage = await opener.pageOrError();
            if (openerPage instanceof page_1.Page && !openerPage.isClosed())
                openerPage.emit(page_1.Page.Events.Popup, page);
        });
    }
