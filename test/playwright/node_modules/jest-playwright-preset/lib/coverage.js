...
exports.saveCoverageToFile = async (coverage) => {
    await fsAsync.writeFile(path_1.default.join(COV_MERGE_DIR, `${uuid.v4()}.json`), JSON.stringify(coverage));
};
exports.saveCoverageOnPage = async (page, collectCoverage) => {
    if (!collectCoverage) {
        console.warn('jest-playwright: saveCoverage was called but collectCoverage is not true in jest-playwright.js');
        return;
    }
